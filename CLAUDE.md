...
- `doReveal()` / `doNewRound()` / the Start button handler — **no client-side optimistic mutation** of `revealed`/`round`/`started` (unlike `doVote`, which is a personal choice and stays optimistic). Each calls its atomic RPC (`revealRound()`/`newRound()`/`startGame()`) and only applies `currentRoom`'s shared fields once the RPC returns the server-confirmed `state` — same trust level as a realtime event from another player's click. `doNewRound()` calls `maybeResetOwnVoteForNewRound()` after applying the confirmed round, using the pre-call `currentRoom.round` as the RPC's expected-round argument
