...
## Supabase
- **URL:** `https://xdynwgiccfaogmwquadp.supabase.co`
- **Anon key:** `eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhkeW53Z2ljY2Zhb2dtd3F1YWRwIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODI3NDkzMjUsImV4cCI6MjA5ODMyNTMyNX0.c6guWNV6WMinbWWCwbF_POvqv1wurhjel-jHtxodj7E`
- **Region:** `eu-central-1` (Frankfurt) — this is a *replacement* project created to test whether region was the cause of the connection stalls/resets documented below; the original project (`xlxkgcbckbjppatmirhc`, region never actually confirmed in the Dashboard — "eu-west-1" in earlier notes was an unverified assumption, not a checked fact) was deleted. `db/supabase_setup.sql` was re-run against this project from scratch.
...
- Verify in the Supabase dashboard (**Database → Publications**, NOT "Database → Replication" — that page is for optional read replicas/Pipelines and will never show `rooms`/`votes`) that the `supabase_realtime` publication includes **both** `rooms` and `votes` — `db/supabase_setup.sql` does this via `alter publication supabase_realtime add table ...`. The same check can be run directly: `select * from pg_publication_tables where pubname = 'supabase_realtime';` in the SQL Editor.
...
- **Project was recreated in `eu-central-1` (Frankfurt) to test whether region explained the RU-side connection stalls/resets** — whether this actually helped is unverified (the old project's real region was never confirmed, so the comparison is inconclusive either way). If multi-second stalls/resets persist on the new project too, region is ruled out and the remaining suspects are RU-side ISP/DPI interference (e.g. with QUIC/HTTP-3, raised earlier in this project's history) — something outside this codebase's control.
- **GitHub Actions deploy needs the CURRENT project's credentials** — `VITE_SUPABASE_URL`/`VITE_SUPABASE_KEY` under Settings → Secrets and variables → Actions → **Variables** must match whatever Supabase project is live right now (`.env`, gitignored, holds the same two for local dev — keep both in sync any time the project changes, as it just did for the Frankfurt move). Pages → Source must also be **"GitHub Actions"**; until both are right, `.github/workflows/deploy.yml` either fails outright or deploys against a stale/wrong project.
