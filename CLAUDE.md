...
- `resyncRoom()` — called every time the channel's status callback reports `SUBSCRIBED` (initial connect *and* every reconnect after a drop). Supabase Realtime never replays events missed while disconnected, so this does one direct `loadRoom`/`loadVotes` read and applies it outright (bypassing `lastAppliedUpdatedAt`, then resetting it to `null`) to pick up anything that happened during the outage. See Bugs fixed below.
